...
class Detection:
    ...
    def set_loader(self,loader_instance):
        ...
        self.run_image(image_path=self.loader_instance.IMAGE_DIR+"rgb.jpg")


    def get_planning(self,path):
    ...
    def is_in_list(self,word,list):
        for obj in list:
            if self.compare_two_words(word, obj) != None and self.compare_two_words(word, obj) > 0.600:
                return True
        return False
    ...
    def compare_two_list_of_objects(self,position_in_image_first,position_in_image_second,relation,object_first,object_second):
        if position_in_image_first == {} or position_in_image_second == {}:
            return
        if "on" in relation:
            # print(position_in_image_first)
            # print(position_in_image_second)
            min_distance_y = 10000
            for key_first in position_in_image_first.keys():
                for key_second in position_in_image_second.keys():

                    min_distance_x_bb = abs(position_in_image_first[key_first]['x'] - position_in_image_second[key_second]['x'])
                    min_distance_y_bb = abs(position_in_image_first[key_first]['y'] - position_in_image_second[key_second]['y'])
                    # print(self.dict_detections[key_first]['label'])
                    # print(self.dict_detections[key_second]['label'])
                    if min_distance_y > min_distance_y_bb:
                        # print("minimo")
                        index_first = key_first
                        index_second = key_second

                        min_distance_y = min_distance_y_bb
            self.data_reordered[index_first]['label'] = object_second
            self.data_reordered[index_second]['label'] = object_first
            # print(index_first)
            # print(index_second)
        if "left" in relation:
            min_distance_x = 3000
            ...
            self.data_reordered[index_second]['label'] = 
    ...
    def run_image(self,image_path):
        ...
        planning = self.get_planning(self.loader_instance.AGENTS+"results_dif_final.pkl")
        object_relations = planning['enviroment_info'].split('\n')
        ...
        for i, (bbox,score,cls_id) in enumerate(zip(bboxs[0], scores, labels_idx[0])):
            ...
            if score > 0.05:
                ...
                if "olbox" in label and score <0.30:
                    continue
                if "elf" in label and score >0.36:
                    continue
                ...
                for mask in masks:
                    ...
                    with open(f'{self.loader_instance.DUMP}box{index_detection}.pkl', 'wb') as f:
                        pickle.dump(binary_mask, f, protocol=2)
                    overlay = cv2.addWeighted(overlay, 1, binary_mask, 0.5, 0)
                    cv2.imwrite(self.loader_instance.IMAGE_DIR+f"rgb_{index_detection}.jpg",overlay)
                    overlay_ = cv2.addWeighted(overlay_, 1, binary_mask, 0.5, 0)
                index_detection += 1
        cv2.imwrite(self.loader_instance.IMAGE_DIR+"mask.jpg", overlay_)
        cv2.imwrite(self.loader_instance.IMAGE_DIR+"yolo.jpg", image_yolow)
        ...
        with open(self.loader_instance.DUMP+"detection.pkl", 'wb') as f:
            pickle.dump(self.data_reordered, f, protocol=2)
        cv2.imwrite(self.loader_instance.IMAGE_DIR+"rgb2_bb.jpg", image_with_bbox)
